...
client.on('interactionCreate', async (interaction) => {
  await analytics.trackInteractions(interaction, (interaction) => {
    if (interaction.isCommandInteraction()) return interaction.data.name;
    else if (interaction.isComponentInteraction() || interaction.isModalSubmitInteraction()) {
      if (
        /\d{17,19}/g.test(
          (interaction as ComponentInteraction | ModalSubmitInteraction).data.customID,
        )
      )
        return 'this_awesome_button';
      else return (interaction as ComponentInteraction | ModalSubmitInteraction).data.customID;
    }
    return '';
  });
