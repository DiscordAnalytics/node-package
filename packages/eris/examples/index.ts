...
import { Client, CommandInteraction, ComponentInteraction, Constants, ModalSubmitInteraction } from 'eris';
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction instanceof CommandInteraction) {
    if (interaction.data.name === 'test') {
      const option = interaction.data.options?.find((o) => o.name === 'test') as { value: string, type: number, name: string } | undefined;

      if (option && option.value === 'button') await interaction.createMessage({
        content: 'Test button',
        components: [{
          type: 1,
          components: [{
      ...
      else if (option && option.value === 'select') await interaction.createMessage({
        content: 'Test select',
        components: [{
          type: 1,
          components: [{
      ...
      else if (option && option.value === 'modal') await interaction.createModal({
        custom_id: `my_modal`,
        title: 'My modal',
        components: [{
          type: 1,
