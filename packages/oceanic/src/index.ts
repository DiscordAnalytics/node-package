...
export default class DiscordAnalytics extends AnalyticsBase {
    ...
    setInterval(
      async () => {
        ...
        const userCount = this._client.guilds.reduce(
          (a: number, g: any) => a + (g.memberCount || 0),
          0,
        );
  ...
  public async trackInteractions(
    ...
    this.updateOrInsert(
      ...
      () => ({
        ...
        icon:
          interaction.guild && interaction.guild.icon
  ...
  public trackEvents(
    ...
    this._client.on("guildCreate", async (guild: any) =>
      this.trackGuilds("create"),
    );
    this._client.on("guildDelete", async (guild: any) =>
      this.trackGuilds("delete"),
    );
  }
}
